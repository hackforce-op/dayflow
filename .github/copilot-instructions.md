...
### ✅ 2026-XX-XX
### ✅ 功能/修复名称
- 修改的文件：`path/to/file.dart`
- 具体改动：描述你做了什么
- 验证：`flutter test` 通过 / `flutter analyze` 无 error
```

### 2. 代码规范
- **必须添加中文注释**：所有新增代码都要写清晰的中文注释
- 注释应说明：功能目的、参数含义、返回值说明
- 保持代码简洁，避免过度工程化
- 必须优先保证用户体验流畅！
- 遵循 Dart 官方代码风格（dart format）

### 3. 项目结构
- `lib/features/` - 按功能模块划分（auth、diary、planner 等）
- `lib/core/` - 核心基础设施（router、theme、supabase 等）
- `lib/shared/` - 共享组件（database、widgets、utils 等）

### 4. 技术栈
- Flutter 3.29+ / Dart 3.7+
- Riverpod 状态管理
- Supabase 后端服务（Auth + Database + Storage）
- Drift ORM（本地数据库）
- go_router 路由管理
...
- 保持 PR 描述清晰，说明改动原因和影响范围
- 创建新文件时说明原因和用途

### 6. 数据库变更
- 每次 schema 变更需在 `completed.md` 中注明
- 提供对应的 SQL 迁移语句或说明
- 提醒需要运行的命令（如 `dart run build_runner build`）

### 7. 环境配置
- 本地配置写入 `env.json`（已加入 .gitignore）
- 敏感信息不要提交到仓库
- 首次运行需配置 Supabase URL 和 ANON KEY

### 8. 已知限制
- 某些平台特性需要特殊处理（如 Linux 桌面端定位）
- 某些依赖存在平台兼容性问题需注意

---
# 以下是追加的指令，请在不违反上面的规则情况下，尽可能遵守以下规范要求！
